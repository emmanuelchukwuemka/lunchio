                    ...
                    <div>
                        ...
                        <div class="space-y-1">
                            ...
                            <a href="{{ route('calendar.index') }}" class="flex items-center px-3 py-2 rounded-xl text-sm font-semibold transition-all duration-300 {{ request()->routeIs('calendar.*') ? 'bg-brand-50 text-brand-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                                <svg class="mr-3 h-5 w-5 {{ request()->routeIs('calendar.*') ? 'text-brand-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                Content Calendar
                            </a>
                            
                            @php
                                $hasWebsite = Auth::user()->hasRole(['admin', 'staff']) || Auth::user()->orders()->whereHas('package.features', function($q) {
                                    $q->where(function($query) {
                                        $query->where('name', 'like', '%Website%')
                                              ->orWhere('pillar', 'like', '%Website%');
                                    })->where('package_feature.included', true);
                                })->exists();
                            @endphp

                            @if($hasWebsite)
                                <a href="{{ route('website-builder') }}" class="flex items-center px-3 py-2 rounded-xl text-sm font-semibold transition-all duration-300 {{ request()->routeIs('website-builder') ? 'bg-brand-50 text-brand-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                                    <svg class="mr-3 h-5 w-5 {{ request()->routeIs('website-builder') ? 'text-brand-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                    Website Builder
                                </a>
                            @endif
                        </div>
